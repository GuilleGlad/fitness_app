...
const getClients = async (req, res) =>{
    try{
        ...
        if(role !== 'admin'){
            [rows] = await pool.execute("SELECT *, users.id as user_id, users.status as status_cuenta, client_profiles.status as client_profiles_status FROM users INNER JOIN client_profiles ON client_profiles.user_id = users.id WHERE client_profiles.trainer_id = ? AND users.id <> 1",[client_id]);
        }else{
            [rows] = await pool.execute("SELECT *, users.id as user_id, users.status as status_cuenta, client_profiles.status as client_profiles_status FROM users LEFT JOIN client_profiles ON client_profiles.user_id = users.id WHERE 1 = 1 AND users.id <> 1",[client_id]);
        }
        return res.status(200).json({
